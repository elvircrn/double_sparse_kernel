...
template <typename Config> struct SpmmKernel {
  ...
  typedef ComputeUtilsSpMM<typename Config::DenseValue, Config::kBlockItemsK,
                           Config::kBlockItemsX, Config::kBlockWidth>
      Computer;

  typedef Barrier<Config::kBlockItemsY, Config::kBlockWidth> Barrier;

  typedef OutputTileSpMM<typename Config::DenseValue, Config::kBlockItemsX,
                         Config::kBlockWidth>
      OutputTileSpMM;
...
cudaError_t CudaSpmmBiasRelu(
  ...
  if (n == 1) {
    typedef SpmmConfig<half2, half2, half2, 1, 32, 32, 32, 1> Config;
    return CudaSpmmEx<Config>(m, k, n, nonzeros, row_indices, values,
                              row_offsets, column_indices, dense_matrix, bias,
                              output_matrix, stream);
  } else if (n < 16) {
