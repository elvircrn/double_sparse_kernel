...
std::vector <torch::Tensor> sputnik_spmm_cuda_fp32(
  ...
  sputnik::CudaSpmm(
	  ...
	  /*n=*/ N,
	  /*nonzeros=*/ nnz,
	  /*row_indices=*/ A_row_indices.data_ptr<int>(),
	  /*values=*/ A_val.data_ptr<float>(),
	  /*row_offsets=*/ A_row_offsets.data_ptr<int>(),
	  /*column_indices=*/ A_col_indices.data_ptr<int>(),
	  /*dense_matrix=*/ B.data_ptr<float>(),
	  /*output_matrix=*/ C.data_ptr<float>(),
...
std::vector <torch::Tensor> sputnik_spmm_cuda_fp16(
  ...
  sputnik::CudaSpmm(
	  ...
	  /*nonzeros=*/ nnz,
	  /*row_indices=*/ A_row_indices.data_ptr<int>(),
	  /*values=*/ (half2 *)A_val.data_ptr<at::Half>(),
	  /*row_offsets=*/ A_row_offsets.data_ptr<int>(),
	  /*column_indices=*/ (short2 *)A_col_indices.data_ptr<short>(),
	  /*dense_matrix=*/ (half2 *)B.data_ptr<at::Half>(),
	  /*output_matrix=*/ (half2 *)C.data_ptr<at::Half>(),
	  /*stream=*/ at::cuda::getCurrentCUDAStream().stream()
  );
...
std::vector <torch::Tensor> sputnik_sddmm_cuda_fp32_benchmark(
  ...
  sputnik::CudaSddmm(
	  ...
	  /*n=*/ N,
	  /*nonzeros=*/ nnz,
	  /*row_indices=*/ M_row_indices.data_ptr<int>(),
	  /*row_offsets=*/ M_row_offsets.data_ptr<int>(),
	  /*column_indices=*/ M_col_indices.data_ptr<int>(),
	  /*lhs_matrix=*/ A.data_ptr<float>(),
	  /*rhs_matrix=*/ BT.data_ptr<float>(),
	  /*output_values=*/ C_val.data_ptr<float>(),
	  /*stream=*/ stream
  );

   AT_CUDA_CHECK(cudaStreamSynchronize(stream));
   t.data_ptr<float>()[0] = timer.end();
...
// 	  N,
// 	  nnz,
// 	  A_val.data_ptr<float>(),
// 	  A_row_offsets.data_ptr<int>(),
// 	  A_col_indices.data_ptr<int>(),
// 	  AT_val.data_ptr<float>(),
// 	  AT_row_offsets.data_ptr<int>(),
// 	  AT_col_indices.data_ptr<int>(),
// 	  CUDA_R_32F,
...
// 	  N,
// 	  nnz,
// 	  A_val.data_ptr<at::Half>(),
// 	  A_row_offsets.data_ptr<int>(),
// 	  A_col_indices.data_ptr<int>(),
// 	  AT_val.data_ptr<at::Half>(),
// 	  AT_row_offsets.data_ptr<int>(),
// 	  AT_col_indices.data_ptr<int>(),
// 	  CUDA_R_16F,
// 	  CUSPARSE_ACTION_NUMERIC,
