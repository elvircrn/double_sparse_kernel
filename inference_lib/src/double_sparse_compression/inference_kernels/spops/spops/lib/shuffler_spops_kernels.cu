...
std::vector <torch::Tensor> csr_cuda_add(
  ...
  if constexpr (std::is_same<T, __nv_bfloat16>::value) {
	e::spadd_csr_v1<T, T, INDEX_TYPE, ACCUMULATOR_TYPE><<<M, 32>>>(
		M,
			N,
			A_row_offsets.data_ptr<ACCUMULATOR_TYPE>(),
			A_row_indices.data_ptr<INDEX_TYPE>(),
			A_col_indices.data_ptr<INDEX_TYPE>(),
			reinterpret_cast<__nv_bfloat16*>(A_val.data_ptr<at::BFloat16>()),
	...
	e::spadd_csr_v1<half, half, INDEX_TYPE, ACCUMULATOR_TYPE><<<M, 32>>>(
		M,
			...
			A_row_indices.data_ptr<INDEX_TYPE>(),
			A_col_indices.data_ptr<INDEX_TYPE>(),
			reinterpret_cast<half*>(A_val.data_ptr<at::Half>()),
